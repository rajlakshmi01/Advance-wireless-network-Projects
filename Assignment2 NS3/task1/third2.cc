...
NS_LOG_COMPONENT_DEFINE("WirelessLANAdhocMode");//modified
...
{
    bool verbose = true;
    uint32_t nWifi = 6; //modified
    bool tracing = false;
    bool useRts = true; //added to force utilization of RTS/CTS
    ...
    WifiHelper wifi;
    wifi.SetStandard(WIFI_STANDARD_80211g); //added
    wifi.SetRemoteStationManager("ns3::AarfWifiManager"); //added
...
"Ssid", SsidValue (ssid));  //modified
    ...
    phy.EnablePcap (useRts ? "rtscts-AT21" : "nortscts-AT21", staDevices.Get (2), true);
    std::ostringstream oss;
    oss <<"/NodeList/" << wifiNodes.Get (2)->GetId ()
